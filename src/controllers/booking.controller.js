...
const searchAvailableFields = async (req, res, next) => {
    try {
        ...
        res.json({ success: true, data: { venues: results } });
    } catch (error) {
        next(error);
    }
};
...
const createBooking = async (req, res, next) => {
    try {
        ...
        const depositRate = parseFloat(process.env.DEFAULT_DEPOSIT_RATE) || 0.1;
...
const confirmBooking = async (req, res, next) => {
    try {
        ...
        if (booking.customerId !== req.user.id) return res.status(403).json({ success: false, message: 'Access denied' });
        ...
        const payAmount = booking.paymentMethod === 'ONLINE' ? booking.totalPrice : booking.depositAmount;
        ...
        res.json({ success: true, message: 'Booking confirmed', data: { booking: updated } });
    } catch (error) {
        next(error);
    }
};
...
const cancelBooking = async (req, res, next) => {
    try {
        ...
        if (!isCustomer && !isOwner) return res.status(403).json({ success: false, message: 'Access denied' });
        ...
        res.json({ success: true, message: 'Booking cancelled', data: { booking: updated, cancellationFee } });
    } catch (error) {
        next(error);
    }
};
        ...
        const [bookings, total] = await Promise.all([
            prisma.booking.findMany({
                ...
                include: {
                    field: {
                        include: {
                            venue: { select: { id: true, name: true, address: true, images: true } },
                        },
                    },
                    payments: true,
                    review: true,
...
const venueBookings = async (req, res, next) => {
    try {
...
const fieldBookedSlots = async (req, res, next) => {
    try {
        ...
        res.json({ success: true, data: { bookedSlots: bookings } });
    } catch (error) {
        next(error);
    }
};
