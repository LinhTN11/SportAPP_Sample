...
const listRoomMessages = async (req, res, next) => {
    try {
        ...
        await prisma.message.updateMany({
            where: { roomId: req.params.roomId, senderId: { not: req.user.id }, isRead: false },
            data: { isRead: true },
        });
...
const createOrGetDirectRoom = async (req, res, next) => {
    try {
        ...
        if (existingRoom) {
            return res.json({ success: true, data: { room: existingRoom.room, isNew: false } });
        }
        ...
        res.status(201).json({ success: true, data: { room, isNew: true } });
    } catch (error) {
        next(error);
    }
};
